...
class IPConfigManager:
    ...
    def _perform_summary(self) -> tuple[bool, str]:
        self.print_summary()
        return True, ""

    def _perform_clear(self) -> tuple[bool, str]:
        if not self.clear_ips():
            return False, "Failed to clear IPs."
        return True, ""

    def _collect_ips_to_add(self, options) -> list:
        ips_to_add = []
        if options.get("file"):
            file_ips = self.load_ips_from_file(options["file"])
            ips_to_add.extend(file_ips)
        if options.get("ips"):
            ips_to_add.extend(options["ips"])
        return ips_to_add

    def _perform_add(self, ips_to_add: list, options) -> tuple[bool, str]:
        if not ips_to_add:
            return True, ""
        ...
        if ips_to_add:
            ...
            if not success:
                ...
                return False, "Some batches may have failed."
        return True, ""

    def run_from_options(self, options) -> tuple[bool, str]:
        """Run the requested operation and return (success, message)."""
        try:
            if options.get("list"):
                return self._perform_list()
            if options.get("summary"):
                return self._perform_summary()
            if options.get("clear"):
                ok, msg = self._perform_clear()
                if not ok:
                    return ok, msg

            ips_to_add = self._collect_ips_to_add(options)
            ok, msg = self._perform_add(ips_to_add, options)
            if not ok:
                return ok, msg

            if (
                not options.get("list")
                and not ips_to_add
                and not options.get("clear")
                and not options.get("summary")
            ):
                logger.warning("No operation specified. Use --help for options.")
                return False, "No operation specified. Use --help for options."

            return True, ""
        except Exception as e:
            logger.exception("Fatal error during IP configuration operation")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            success, message = manager.run_from_options(options)
            if not success:
                if message:

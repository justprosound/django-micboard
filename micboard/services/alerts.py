...
class AlertManager:
    ...
    def check_device_offline_alerts(self, channel: RFChannel) -> None:
        ...
        if not channel.chassis.is_active_at_time():
            self._create_device_offline_alert(channel)
        else:
            # Check if active wireless unit is offline (no recent updates)
    ...
    def _create_alert(
        ...
        Args:
            channel: RFChannel that triggered the alert
            user: User to notify
...
def check_device_offline_alerts(channel: RFChannel):
    """Convenience function to check offline alerts for a channel.

    Args:
        channel: RFChannel instance
    """

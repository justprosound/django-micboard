...
- **MicboardSettingsProxy** (`conf.py`) — Runtime settings proxy merging Django settings with DB-backed config.
...
- **Background Tasks** — Discovery, polling, health checks, and WebSocket subscriptions run via Celery tasks in `micboard/tasks/<domain>/`.
...
5. **Dual settings proxy** — `conf.py` (MicboardSettingsProxy) and `services/shared/settings_registry.py` overlap; direct `settings.MICBOARD_CONFIG` reads scattered across codebase.
6. **Thin test coverage** — 33 of 38 service files untested; no test factories; no integration or E2E tests.
7. **Compat shim** — `micboard/manufacturers/` is a backward-compat shim that should be removed.
...
| Tasks (Celery) | `micboard/tasks/<domain>/` |
| Manufacturer plugins | `micboard/integrations/<manufacturer>/` |
| View layer (non-admin) | `micboard/views/` |
| Settings proxy | `micboard/conf.py` |
| DB settings models | `micboard/models/settings/registry.py` |
| Settings resolution | `micboard/services/shared/settings_registry.py` |
| Legacy compat shim | `micboard/manufacturers/` |
| Tests | `tests/` (flat) |
| Fixtures | `micboard/fixtures/` |
| Celery app | `micboard/celery_app.py` |

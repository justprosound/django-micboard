...
Your goal is to create a new implementation plan file for new features, refactoring existing code, or upgrading packages, design, architecture, or infrastructure. Your output must be machine-readable, deterministic, and structured for autonomous execution.
...
The status of the implementation plan must be clearly defined in the front matter and must reflect the current state of the plan. The status can be one of the following:
...
1. Fill in all required sections with specific, actionable content
2. Use the standardized identifier prefixes (REQ-, TASK-, DEP-, etc.)
3. Include measurable completion criteria for each phase
4. Ensure all file paths and code references are precise
5. Validate that all sections follow the template structure exactly
6. Update the status and last_updated fields appropriately

...
Read assignments through `visible_to(PerformerAssignment, user=...)`, which applies tenant
and monitoring-group visibility and returns an ordinary queryset. Filter and plan
relationships on it directly, for example `.filter(is_active=True)` and
...
2. The performer must be returned by `visible_to(Performer, user=...)`.
3. The wireless unit must be returned by `visible_to(WirelessUnit, user=...)`.
...
assignments = visible_to(PerformerAssignment, user=request.user)
performers = visible_to(Performer, user=request.user)
```
...
The assignment list view combines `visible_to()` with `select_related()` for performer, wireless unit, and monitoring group. Services that need chassis/location details should extend that explicit relationship plan rather than triggering per-row queries.

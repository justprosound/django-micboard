...
class DeviceTestingFramework:
        ...
        try:
            receiver, created = WirelessChassis.objects.get_or_create(
                ...
                defaults={
                    ...
                    "firmware_version": api_device.get("properties", {}).get(
                        "firmware_version", ""
                    ),
                    "is_active": True,
                },
    ...
    def simulate_polling(self, receivers: List["WirelessChassis"]):
        ...
        try:
            ...
            for receiver in receivers:
                api_device = next(
                    (d for d in api_devices if d.get("id") == receiver.api_device_id), None
                )
                ...
                if api_device:
                    ...
                    logger.info(
                        f"  Firmware: {api_device.get('properties', {}).get('firmware_version', 'N/A')}"
                    )
                    logger.info(
                        f"  Serial: {api_device.get('properties', {}).get('serial_number', 'N/A')}"
                    )
...
After this test passes:
  ...
  3. Web UI: python manage.py runserver
    ...
    parser.add_argument("--cleanup", action="store_true", help="Remove test devices from database")

        ...
        if self.needs_any:
            ...
            if not has_any_import:
                ...
                for i, stmt in enumerate(updated_node.body):
                    if m.matches(stmt, m.SimpleStatementLine(body=[m.ImportFrom(module=m.Name("__future__"))])) or (isinstance(stmt, cst.SimpleStatementLine) and isinstance(stmt.body[0], cst.Expr) and isinstance(getattr(stmt.body[0], "value", None), cst.SimpleString) and i == 0):
                        insert_idx = i + 1

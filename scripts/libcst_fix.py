...
class TypingFixer(cst.CSTTransformer):
    ...
    def leave_FunctionDef(  # noqa: N802
        ...
        if isinstance(new_kwarg, cst.Param) and new_kwarg.annotation is None:
            self.needs_any = True
            new_kwarg = new_kwarg.with_changes(
                annotation=cst.Annotation(annotation=cst.Name("Any"))
            )

        new_params_node = updated_node.params.with_changes(
            params=new_params, kwonly_params=new_kwonly, star_arg=new_star_arg, star_kwarg=new_kwarg
        )
    ...
    def leave_Module(self, original_node: cst.Module, updated_node: cst.Module) -> cst.Module:  # noqa: N802, C901
        if self.needs_any:
            ...
            for stmt in updated_node.body:
                if m.matches(
                    ...
                    for import_stmt in stmt.body:
                        if (
                            isinstance(import_stmt, cst.ImportFrom)
                            and getattr(import_stmt.module, "value", "") == "typing"
                        ):
            ...
            if not has_any_import:
                ...
                for i, stmt in enumerate(updated_node.body):
                    if m.matches(
                        ...
                        and isinstance(getattr(stmt.body[0], "value", None), cst.SimpleString)
                        and i == 0
                    ):
                        insert_idx = i + 1

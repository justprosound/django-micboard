...
class DiscoveryMonitor:
    ...
    def format_device_info(self, device: Dict[str, Any]) -> str:
        ...
        fw = device.get("properties", {}).get("firmware_version", "Unknown") if isinstance(device.get("properties"), dict) else "Unknown"
        serial = device.get("properties", {}).get("serial_number", "") if isinstance(device.get("properties"), dict) else ""
    ...
    def run(self, duration: Optional[int] = None):
        ...
        try:
            while True:
                ...
                if new_devices:
                    logger.info(f"[{self._timestamp()}] 🆕 {len(new_devices)} NEW DEVICE(S) DISCOVERED:")

#!/usr/bin/env python
"""
Shure System API: Bulk IP Discovery Configuration
...
Examples:
  ...
  python scripts/shure_configure_discovery_ips.py --clear --file ips.txt
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List currently configured IPs"
    )
    parser.add_argument(
        "--file",
        metavar="PATH",
        help="Load IPs from file (one per line or space-separated)"
    )
    parser.add_argument(
        "--ips",
        nargs="+",
        metavar="IP",
        help="Add specific IPs"
    )
    parser.add_argument(
        "--clear",
        action="store_true",
        help="Clear all configured IPs before adding new ones"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate IPs before adding (slower but safer)"
    )
    parser.add_argument(
        "--summary",
        action="store_true",
